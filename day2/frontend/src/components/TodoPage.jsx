...
export default function TodoPage({ user, onLogout, onUpdateUser }) {
  ...
  ><ListIcon size={SM} /> List
    ...
    /* Progress bar */
  }{todos.length > 0 && <div className="ms-progress-wrap"><div className="ms-progress-header"><span className="ms-progress-label">{completedCount} of {todos.length} tasks completed
                    </span><span className="ms-progress-pct" style={{ color: viewMeta.color }}>{pct === 100 ? "\u{1F389} All done!" : `${pct}%`}</span></div><div className="ms-progress-track"><div
    className={`ms-progress-fill${pct === 100 ? " complete" : ""}`}
    style={{
      width: `${pct}%`,
      backgroundColor: pct === 100 ? void 0 : viewMeta.color
    }}
    role="progressbar"
    aria-valuenow={pct}
    aria-valuemin={0}
